...
if args.f == None:  #error checking: if there is no file chosen
	print("Error: Please choose fasta file (using -f tag) to perform analysis eg. 'massspec.py -f digested_a.fasta'") #print error message
	sys.exit()  #halts the program
file = args.f # assign filename to as variable
if '.fasta' not in file[-6:]: #error checking: if file suffix is not.fasta
	print("Error: please make sure file is named with '.fasta' suffix")  #if not print error
	sys.exit()  # halts the program
...
if not fileCheck.is_file():  # if pathlib cannot find the file
	print("Error: Please check chosen file (-f) exists and is in working directory.") # print error message
	sys.exit()  # halts the program
...
for index in range(0, len(lines),2): # for each of the lines in the range, counting in increments of 2
	...
	missedCleaves = splitHeading[2][8] # missed cleaves 
...
if args.s == 'y':
	statsFile = open(file[:-6]+'.stats', 'w') #initiates a new output file for stats
	statsOverview = open(file[:-6]+'.csv', 'w')
